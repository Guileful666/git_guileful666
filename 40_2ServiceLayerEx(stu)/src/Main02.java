...
public class Main02 {
	...
	public static void main(String[] args) {
		...
		/** 수정 데이터를 담고 있는 Beans 객체 생성 */
		// --> import study.java.myschool.model.Department;
		Student dept = new Student();
		dept.setStudno(20106);
		dept.setName("뚬땜이");
		dept.setUserid("bbb01");
		dept.setGrade(2);
		dept.setIdnum("9902021111111");
		dept.setBirthdate("1999-02-02");
		dept.setTel("055-456-7890");
		dept.setHeight(180);
		dept.setWeight(100);
		dept.setDeptno(102);
		dept.setProfno(9906);
		...
		try {
			//학과 수정하기
			result = studentService.editStudent(dept);
			//저장결과를 데이터베이스에 반영하기
			session.commit();
			//처리 결과를 로그에 기록하기
